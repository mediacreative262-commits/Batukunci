...
function renderChatAttachment(a){
  if(!a?.url)return '';
  if(a.type==='image') return `<a class="chat-media image" href="${escapeAttr(a.url)}" target="_blank" rel="noopener"><img src="${escapeAttr(a.url)}" alt="Foto" loading="lazy"></a>`;
  if(a.type==='video') return `<video class="chat-media video" controls preload="metadata" src="${escapeAttr(a.url)}"></video>`;
  if(a.type==='audio') return `<div class="chat-audio"><span>🎙️ VN</span><audio controls preload="metadata" src="${escapeAttr(a.url)}"></audio></div>`;
  return `<a class="chat-file" href="${escapeAttr(a.url)}" target="_blank" rel="noopener">📎 ${escapeHtml(a.name||'Lampiran')}</a>`;
}
...
async function renderGroups() {
  ...
  const groups=snap.docs.map(d=>({id:d.id,...d.data()})).filter(g=>groupMemberIds(g).includes(currentUser?.id));
...
function renderGroupAdminActions(group){ const admin=groupIsAdmin(group); const btn=document.getElementById('group-edit-btn'); if(btn) btn.classList.toggle('hidden', !admin); const label=document.getElementById('group-chat-admin-label'); if(label) label.textContent=admin?'Admin grup':'Anggota grup'; }
...
async function sendGroupMessage(e){
  ...
  if(!text||!selectedGroupId)return; i.value=''; await sendChatText(selectedGroupId,text);
}
async function sendChatText(chatId,text){
  await db.collection('chats').doc(chatId).collection('messages').add({senderId:currentUser.id,senderName:currentUser.name,text,createdAt:firebase.firestore.FieldValue.serverTimestamp()});
...
async function handleChatMediaSelected(input,mode){
  ...
  try{
    showToast('Mengunggah lampiran…');
    const type=file.type.startsWith('image/')?'image':file.type.startsWith('video/')?'video':'file';
    const safeName=file.name.replace(/[^a-zA-Z0-9._-]/g,'_');
    const path=`chatMedia/${chatId}/${currentUser.id}/${Date.now()}_${safeName}`;
    const snap=await storage.ref(path).put(file,{contentType:file.type}); const url=await snap.ref.getDownloadURL();
    await db.collection('chats').doc(chatId).collection('messages').add({senderId:currentUser.id,senderName:currentUser.name,text:'',attachment:{type,url,name:file.name,size:file.size,mime:file.type},createdAt:firebase.firestore.FieldValue.serverTimestamp()});
    await db.collection('chats').doc(chatId).set({updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});
...
async function toggleVoiceRecording(mode){
  ...
  try{
    ...
    activeVoiceRecorder.onstop=async()=>{
      ...
      try{
        showToast('Mengunggah VN…'); const ext=blob.type.includes('mp4')?'m4a':'webm'; const path=`chatMedia/${chatId}/${currentUser.id}/${Date.now()}_voice.${ext}`;
        const snap=await storage.ref(path).put(blob,{contentType:blob.type}); const url=await snap.ref.getDownloadURL();
        await db.collection('chats').doc(chatId).collection('messages').add({senderId:currentUser.id,senderName:currentUser.name,text:'',attachment:{type:'audio',url,name:'Voice Note',size:blob.size,mime:blob.type},createdAt:firebase.firestore.FieldValue.serverTimestamp()});
