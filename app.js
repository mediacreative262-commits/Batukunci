...
try {
  ...
  window.BK_APP_CHECK_READY = true;
} catch (err) {
  console.error("Batu Kunci App Check init:", err);
  window.BK_APP_CHECK_READY = false;
}
...
function messageHtml(m) { const mine=m.senderId===currentUser?.id; return `<div class="chat-message ${mine?'mine':''}"><div class="chat-avatar">${initials(m.senderName||findUser(m.senderId)?.name||'?')}</div><div><div class="chat-message-meta"><strong>${escapeHtml(m.senderName||findUser(m.senderId)?.name||'Anggota')}</strong><small>${formatChatTime(m.createdAt)}</small></div><div class="chat-bubble">${escapeHtml(m.text||'')}</div></div></div>`; }
...
function ensureDefaultGroup() { if (!currentUser) return db; const ref=db.collection('chats').doc('team-media-kreatif'); return ref.set({type:'group',name:'Tim Media Kreatif',memberIds:TEAM.map(u=>u.id),updatedAt:firebase.firestore.FieldValue.serverTimestamp(),createdBy:'system'},{merge:true}).catch(e=>console.warn('default group',e)); }
async function renderGroups() { const list=document.getElementById('group-list'); if(!list)return; const snap=await db.collection('chats').where('type','==','group').get(); const groups=snap.docs.map(d=>({id:d.id,...d.data()})).filter(g=>(g.memberIds||[]).includes(currentUser?.id)); if(!selectedGroupId && groups[0]) selectedGroupId=groups[0].id; list.innerHTML=groups.length?groups.map(g=>`<button class="chat-list-item ${g.id===selectedGroupId?'active':''}" onclick="selectGroup('${g.id}')"><strong>${escapeHtml(g.name)}</strong><small>${(g.memberIds||[]).length} anggota</small></button>`).join(''):'<div class="chat-empty">Belum ada grup.</div>'; if(selectedGroupId) loadGroupMessages(selectedGroupId); }
function selectGroup(id){selectedGroupId=id;renderGroups();}
function loadGroupMessages(id){if(unsubChatMessages){unsubChatMessages();unsubChatMessages=null;} const box=document.getElementById('group-messages'); const title=document.getElementById('group-chat-title'); if(!box)return; db.collection('chats').doc(id).get().then(d=>{if(title)title.textContent=d.data()?.name||'Grup';}); unsubChatMessages=db.collection('chats').doc(id).collection('messages').orderBy('createdAt','asc').onSnapshot(s=>{box.innerHTML=s.empty?'<div class="chat-empty">Belum ada pesan.</div>':s.docs.map(d=>messageHtml({id:d.id,...d.data()})).join('');box.scrollTop=box.scrollHeight;});}
async function sendGroupMessage(e){e.preventDefault();const i=document.getElementById('group-message-input');const text=(i?.value||'').trim();if(!text||!selectedGroupId)return;i.value='';await db.collection('chats').doc(selectedGroupId).collection('messages').add({senderId:currentUser.id,senderName:currentUser.name,text,createdAt:firebase.firestore.FieldValue.serverTimestamp()});await db.collection('chats').doc(selectedGroupId).set({updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});}
...
async function loadDmMessages(otherId){const box=document.getElementById('dm-messages');const title=document.getElementById('dm-chat-title');const u=findUser(otherId);if(title)title.textContent=u?.name||'Japri';if(unsubChatMessages){unsubChatMessages();unsubChatMessages=null;}const id=dmChatId(currentUser.id,otherId);await db.collection('chats').doc(id).set({type:'dm',memberIds:[currentUser.id,otherId]},{merge:true});unsubChatMessages=db.collection('chats').doc(id).collection('messages').orderBy('createdAt','asc').onSnapshot(s=>{box.innerHTML=s.empty?'<div class="chat-empty">Belum ada pesan. Kirim yang pertama.</div>':s.docs.map(d=>messageHtml({id:d.id,...d.data()})).join('');box.scrollTop=box.scrollHeight;});}
async function sendDirectMessage(e){e.preventDefault();const i=document.getElementById('dm-message-input');const text=(i?.value||'').trim();if(!text||!selectedDmUserId)return;i.value='';const id=dmChatId(currentUser.id,selectedDmUserId);await db.collection('chats').doc(id).set({type:'dm',memberIds:[currentUser.id,selectedDmUserId],updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});await db.collection('chats').doc(id).collection('messages').add({senderId:currentUser.id,senderName:currentUser.name,text,createdAt:firebase.firestore.FieldValue.serverTimestamp()});}
function stopChatMessages(){if(unsubChatMessages){unsubChatMessages();unsubChatMessages=null;}}
function openCreateGroup(){
  const name=prompt('Nama grup baru:'); if(!name?.trim())return;
  const ids=TEAM.filter(u=>confirm(`Masukkan ${u.name} ke grup?`)).map(u=>u.id); if(!ids.includes(currentUser.id))ids.push(currentUser.id);
  db.collection('chats').add({type:'group',name:name.trim(),memberIds:ids,createdBy:currentUser.id,createdAt:firebase.firestore.FieldValue.serverTimestamp()}).then(d=>{selectedGroupId=d.id;showToast('Grup dibuat ✓');renderGroups();}).catch(e=>showToast('Gagal membuat grup: '+e.message));
}
