...
function setupMusic() {
  ...
  const direct = driveDirectAudioUrl(currentUser?.music);
  if (direct) {
    if (audio.dataset.src !== direct) { audio.src = direct; audio.dataset.src = direct; }
    audio.volume = 0.35;
...
function toggleMusic() {
  const audio = document.getElementById('bg-music');
  const btn = document.getElementById('music-toggle');
  if (audio.paused) { audio.play().catch(() => showToast('Gagal muter musik — cek link-nya')); btn.textContent = '🔊'; }
  else { audio.pause(); btn.textContent = '🎵'; }
}

function driveDirectAudioUrl(link) {
  if (!link) return null;
...
function addAsetRow() {
  ...
  div.innerHTML = `
    ...
    <div class="aset-link-actions">
      <a href="https://drive.google.com" target="_blank" rel="noopener" class="btn btn-ghost btn-sm">Buka Drive ↗</a>
...
async function addGalleryItem() {
  ...
  try {
    await db.collection('gallery').add({ label, link, akun });
