...
let uploadAsetRows = [];
let asetRowSeq = 0;
let unsubProjects = null, unsubDrive = null, unsubGallery = null, unsubAlbums = null, unsubUsers = null, unsubNotifications = null, unsubChatMessages = null;
...
const BK_THEMES = {
  original: {
    label: 'Batu Kunci Original',
    desc: 'Akademik Eropa · collegiate · batu kunci',
    glyph: '◆'
  },
  horror: {
    label: 'Horror',
    desc: 'Gelap · crimson · gothic',
    glyph: '☾'
  },
  cool: {
    label: 'Cool',
    desc: 'Midnight · ice blue · tenang',
    glyph: '✦'
  },
  rich: {
    label: 'Rich',
    desc: 'Obsidian · champagne · premium',
    glyph: '◇'
  },
  genz: {
    label: 'Gen Z',
    desc: 'Indigo · pink · playful',
    glyph: '✷'
  },
  futuristic: {
    label: 'Futuristik',
    desc: 'Graphite · cyan · digital',
    glyph: '⌁'
  }
};
...
function initTheme() {
  ...
  const skin = BK_THEMES[savedSkin] ? savedSkin : 'original';
...
function setWorkspaceTheme(skin) {
  if (!BK_THEMES[skin]) skin = 'original';
  document.documentElement.setAttribute('data-skin', skin);
  try { localStorage.setItem('bk-skin', skin); } catch (e) {}
  renderThemePicker();
  applyUserBg();
}
...
function renderThemePicker() {
  ...
  if (!wrap) return;
  const current = document.documentElement.getAttribute('data-skin') || 'original';
  wrap.innerHTML = Object.entries(BK_THEMES).map(([key, t]) =>
    `<button type="button" class="theme-card theme-card-${key} ${key === current ? 'selected' : ''}" onclick="setWorkspaceTheme('${key}')" aria-pressed="${key === current}">
      <span class="theme-card-preview"><i></i><b>${t.glyph}</b></span>
      <span class="theme-card-copy"><strong>${t.label}</strong><small>${t.desc}</small></span>
...
async function loadTeamForLogin() {
  try {
    // Ambil sekali supaya login tidak menunggu listener, lalu pasang listener
    // permanen agar nama/foto/anggota chat selalu mengikuti Firestore.
    ...
    if (!unsubUsers) {
      unsubUsers = db.collection('users').onSnapshot(s => {
        ...
        renderLoginUserOptions();
        if (currentUser) {
          const fresh = findUser(currentUser.id);
          if (fresh) currentUser = { ...fresh, authEmail: currentUser.authEmail, authUid: currentUser.authUid };
          applyUserBg();
      ...
      }, e => console.error('Daftar anggota listener', e));
    }
  } catch (e) {
...
function renderLoginUserOptions() {
  ...
  if (!el) return;
  el.innerHTML = TEAM.map(u => `<option value="${escapeAttr(u.id)}">${escapeHtml(u.name || u.id)}</option>`).join('')
...
function stopLiveData() {
  ...
  // User directory sengaja tetap hidup selama sesi login berikutnya; reset
  // referensi supaya listener tidak dobel setelah login ulang.
...
async function handleDriveFilePicked(e) {
  ...
  try {
    ...
    if (isProfile) {
      const photoUrl = `https://drive.google.com/thumbnail?id=${encodeURIComponent(result.fileId)}&sz=w400`;
...
function renderSettings() {
  renderProfileHeader();
  // Pastikan pilihan tema dirender setiap kali Settings dibuka. Ini juga
  // membuat halaman tahan terhadap cache HTML lama di GitHub Pages.
  renderThemePicker();
...
function messageHtml(m) {
  ...
  const sender=findUser(m.senderId) || (mine ? currentUser : null);
  const senderName=m.senderName || sender?.name || 'Anggota';
  const avatar=avatarHtml(sender || {name:senderName}, 'avatar-chat');
  ...
  const reply=m.replyTo ? `<div class="chat-reply-preview"><strong>${escapeHtml(m.replyTo.senderName||'Anggota')}</strong><span>${escapeHtml(m.replyTo.text||'Lampiran')}</span></div>` : '';
  const replyText=(m.text||m.attachment?.name||'Lampiran').slice(0,120);
  return `<div class="chat-message ${mine?'mine':''}" data-message-id="${escapeAttr(m.id||'')}">
    <div class="chat-avatar-wrap">${avatar}</div>
    <div class="chat-message-main">
      <div class="chat-message-meta"><strong>${escapeHtml(senderName)}</strong><small>${formatChatTime(m.createdAt)}</small></div>
      <div class="chat-bubble">${reply}${attachment}${text}</div>
      <div class="chat-message-actions"><button type="button" class="chat-reply-btn" onclick='startReply(${JSON.stringify({id:m.id||'',senderId:m.senderId||'',senderName, text:replyText}).replace(/</g,"\\u003c")})'>↩ Balas</button></div>
    </div>
  </div>`;
}
...
function startReply(message){
  ...
  replyingTo = {
    ...
    senderName: message.senderName || 'Anggota',
    text: String(message.text || 'Lampiran').slice(0,120)
  };
  const bar=document.querySelector('.chat-reply-bar');
  const name=bar?.querySelector('.chat-reply-name');
  const text=bar?.querySelector('.chat-reply-text');
  if(name) name.textContent=`Membalas ${replyingTo.senderName}`;
  if(text) text.textContent=replyingTo.text;
  bar?.classList.remove('hidden');
  const input=document.querySelector('.chat-panel:not(.hidden) .chat-composer input:not(.chat-file-input)');
  input?.focus();
}
function cancelReply(){
  replyingTo=null;
  document.querySelectorAll('.chat-reply-bar').forEach(x=>x.classList.add('hidden'));
}
function getReplyPayload(){
  if(!replyingTo) return null;
  return {...replyingTo};
}
function withReply(data){
  const r=getReplyPayload();
...
function renderDms(){
  ...
  if(!list)return;
  const users=TEAM.filter(u=>u.id!==currentUser?.id);
  if(!users.length){
    list.innerHTML='<div class="chat-empty">Memuat daftar anggota…</div>';
    return;
  }
  if(!selectedDmUserId || !users.some(u=>u.id===selectedDmUserId)) selectedDmUserId=users[0].id;
  list.innerHTML=users.map(u=>`<button class="chat-list-item ${u.id===selectedDmUserId?'active':''}" onclick="selectDm('${escapeAttr(u.id)}')">${avatarHtml(u,'avatar-sm')}<span><strong>${escapeHtml(u.name)}</strong><small>Pesan pribadi</small></span></button>`).join('');
  if(selectedDmUserId)loadDmMessages(selectedDmUserId);
}
