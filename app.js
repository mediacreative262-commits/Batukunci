...
function initGoogleAuth() {
  if (!window.google || !google.accounts || GOOGLE_CLIENT_ID.startsWith('263094318099-vuhh3e927sscov459ao800u5bf2ecofa.apps.googleusercontent.com')) return;
...
async function handleDriveFilePicked(e) {
  ...
  const row = document.getElementById(rowId);
  const btn = row?.querySelector('.aset-upload-btn');
  ...
  try {
    ...
    if (btn) { btn.disabled = false; btn.textContent = '⬆ Upload Langsung ke Drive'; }
    e.target.value = '';
  }
}
