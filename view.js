...
function initTheme() {
  let mode = null, palette = null;
  try {
    mode = localStorage.getItem('bk-theme-mode') || localStorage.getItem('bk-theme');
    palette = localStorage.getItem(mode === 'dark' ? 'bk-theme-dark' : 'bk-theme-light');
  } catch (e) {}
  mode = mode || ((window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) ? 'dark' : 'light');
  palette = palette || (mode === 'dark' ? 'obsidian' : 'atelier');
...
function toggleTheme() {
  ...
  let palette = null;
  try { palette = localStorage.getItem(next === 'dark' ? 'bk-theme-dark' : 'bk-theme-light'); } catch (e) {}
  document.documentElement.setAttribute('data-theme', next);
  document.documentElement.setAttribute('data-palette', palette || (next === 'dark' ? 'obsidian' : 'atelier'));
  try { localStorage.setItem('bk-theme-mode', next); localStorage.setItem('bk-theme', next); } catch (e) {}
}
initTheme();
